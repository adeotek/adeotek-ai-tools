...
public partial class MsSqlQueryService(
    IOptions<DatabaseServerOptions> databaseServerOptions,
    ILogger<MsSqlQueryService> logger)
    : ISqlQueryService
{
    ...
    private string BuildConnectionString(string? database = null)
    {
        ...
        var connectionStringBuilder = new SqlConnectionStringBuilder(_databaseServerOptions.ConnectionString)
        {
            InitialCatalog = string.IsNullOrEmpty(database) ? "master" : database,
            ConnectTimeout = _databaseServerOptions.ConnectionTimeoutSeconds,
