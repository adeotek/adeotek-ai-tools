# Adeotek AI Tools - Context for Gemini

This document provides high-level context about the Adeotek AI Tools repository for Gemini (both CLI and Web) interactions. It describes the repository structure, general development guidelines, and architectural patterns shared across all projects.

**For project-specific details, see the GEMINI.md file in each project directory:**
- **HTTP Agent**: `/agents/http-agent/GEMINI.md`
- **PostgreSQL MCP Server** (read-only): `/mcp-servers/postgres-mcp/GEMINI.md`
- **PostgreSQL Natural Language MCP**: `/mcp-servers/postgres-nl-mcp/GEMINI.md`

> **📖 Important for Gemini**: When working on a specific project task, **ALWAYS read that project's GEMINI.md file FIRST** using the Read tool before proceeding. This ensures you have the detailed context, architecture patterns, and implementation guidelines specific to that project. See the "Working with Gemini: Important Workflow" section below for details.
...
├── GEMINI.md                # This file - detailed context for Gemini
├── LICENSE                  # MIT License
...
**📄 For detailed documentation**: See `/agents/http-agent/GEMINI.md`
...
**📄 For detailed documentation**: See `/mcp-servers/postgres-mcp/GEMINI.md`
...
**📄 For detailed documentation**: See `/mcp-servers/postgres-nl-mcp/GEMINI.md`
...
5. **Document thoroughly**:
   ...
   - Create `GEMINI.md` in agent directory (detailed context for Gemini - **REQUIRED**)
   - Update this root `GEMINI.md` with a brief project overview and reference to the project's GEMINI.md
   - Update root `README.md` with quick reference

   **Note**: The project-specific `GEMINI.md` is critical for Gemini to work effectively on your project. Include architecture, configuration, development workflow, troubleshooting, and code quality standards.
...
3. **Add documentation**:
   ...
   - Create `GEMINI.md` (detailed context for Gemini - **REQUIRED**)
   - Update this root `GEMINI.md` with a brief overview and reference to the project's GEMINI.md
   - Tool descriptions and usage examples
   - Configuration instructions
   - Docker deployment guide

   **Note**: The project-specific `GEMINI.md` is critical for Gemini to work effectively on your project. Include MCP tools documentation, configuration, security features, testing, and troubleshooting.
...
For project-specific commands, see the GEMINI.md file in each project directory. General commands:
...
1. **Understand the context**: Read the relevant README and this GEMINI.md
2. **Follow patterns**: Use existing agents/servers as templates
3. **Test thoroughly**: Build, run, and test your changes
4. **Document**: Update README files and this GEMINI.md
...
All projects include comprehensive test suites. For project-specific testing instructions, see each project's GEMINI.md file.
...
For project-specific troubleshooting, see each project's GEMINI.md file.
...
## Working with Gemini: Important Workflow

### Before Starting Any Task

**CRITICAL: When working on a specific project, ALWAYS read that project's GEMINI.md file FIRST before proceeding with the task.**
...
2. **Read the project's GEMINI.md**: Use the Read tool to load the project-specific GEMINI.md file
...
- Task: "Fix a bug in the HTTP agent SSL verification"
  → **First read** `/agents/http-agent/GEMINI.md` to understand the SSL implementation
  → Then proceed with debugging

- Task: "Add a new validation rule to postgres-mcp"
  → **First read** `/mcp-servers/postgres-mcp/GEMINI.md` to understand query validation
  → Then implement the new rule

- Task: "Optimize a query in postgres-nl-mcp"
  → **First read** `/mcp-servers/postgres-nl-mcp/GEMINI.md` to understand AI integration
  → Then optimize the query

**Why This Matters**:
- Each project has specific architecture patterns, coding standards, and security considerations
- Project-specific GEMINI.md files contain detailed implementation guidance not in this root file
- Reading project context first prevents mistakes and ensures consistency
- This root GEMINI.md is for repository-wide guidance only

### When to Use Which GEMINI.md File

**Use Root GEMINI.md** (`/GEMINI.md`) for:
...
**Use Project-Specific GEMINI.md** for:
...
For questions about specific projects, Gemini will automatically reference the appropriate project-specific GEMINI.md file:

- "How does the HTTP agent handle SSL certificates?" → See `/agents/http-agent/GEMINI.md`
- "What LLM providers does the HTTP agent support?" → See `/agents/http-agent/GEMINI.md`
- "How does query validation work in postgres-mcp?" → See `/mcp-servers/postgres-mcp/GEMINI.md`
- "What's the difference between postgres-mcp and postgres-nl-mcp?" → See `/mcp-servers/postgres-mcp/GEMINI.md` or `/mcp-servers/postgres-nl-mcp/GEMINI.md`
- "How do I configure the PostgreSQL Natural Language MCP server?" → See `/mcp-servers/postgres-nl-mcp/GEMINI.md`
- "How does SQL injection prevention work in postgres-nl-mcp?" → See `/mcp-servers/postgres-nl-mcp/GEMINI.md`

Gemini has full context from this document and can help with repository-wide development, architecture decisions, and will reference project-specific GEMINI.md files for detailed project-specific questions.
